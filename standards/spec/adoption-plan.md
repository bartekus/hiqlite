...
W-16 closed the pass as `016-derive-macros` and
`017-examples-as-documentation`, the latter closing F-015 outright.

**Coverage after the whole pass: 125/236 (53.0%)**, from 79/230 (34.3%) at its
start. The numerator moved by 46 files across six waves; the denominator moved by
...
| W-04 | cache log store contract repair: F-021 to F-024, F-029 and F-047, and reconciling the two defects `007` omits into it | `007` | not started; **proposal traced 2026-09-21**, `standards/spec/cache-log-repair-proposal.md`, which authorizes nothing | two characterization tests, which pin the current behavior including the exclusive purge; the proposal's section 5.1 states that both must be replaced rather than extended | **two, both raised by the proposal**: the lock ordering in its 4.2, and bounded per-method tests (5.2) against OpenRaft's own conformance suite (5.3), which would prove the whole trait but needs a `StoreBuilder` reaching into `006`'s unit | none | the contract matches the locked trait, the replaced acceptance is carried through the governed mechanism rather than left asserting the old outcome, and `007` records or closes the defects it omits |
