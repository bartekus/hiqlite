...
**Known defects retained.** F-017 is resolved: its cause is F-051, repaired by `032` on 2026-09-22, and the suite completes.
...
#### Rung 0 is **not** blocked. The 2026-09-19 probe was read wrong (corrected 2026-09-21)

**Superseded by `028-enforcement-readiness-and-acceptance-control`.** The
section below is kept as written, because it is the record of what was probed
and concluded, and because the correction is only legible beside it.

What it got wrong is one thing, and everything else followed from it: the pinned
tool matches `index.resolver_exclusions` entries as path **components**, not as
path prefixes. The probe tried `"hiqlite/static"` and `"dashboard/src/spow"`,
which are not components of anything, and correctly observed no effect.
`"static"` and `"spow"` are components, and they do exactly what this rung asks
for: the denominator goes from 239 to 223, all sixteen files leave, the numerator
does not move, and `coupling.bypass_prefixes` stays empty so no ownership check
on authored source is touched. Recorded as F-097.

The consequences recorded below therefore do not hold. Rung 2 is not unreachable
by configuration; it is unreachable because seventy-one files are unclaimed, and
that is a different problem with a different answer. The recommended pin upgrade
is not needed, and would not have helped: the tool's current head still has no
separate denominator-exclusion key.

**The original entry, unchanged, follows.**
...
| W-14 | denominator exclusion for the 12 generated `.js` and 4 vendored files (F-016) | `000` owns `spec-spine.toml`; applied by **`028-enforcement-readiness-and-acceptance-control`**, 2026-09-21 | **delivered**: `index.resolver_exclusions` gains `"static"` and `"spow"`, as path components | measured: denominator 239 to 223, numerator unchanged, `C-001` still refusing an authored claimed file, and a vendored edit still staling the index | **answered**: no pin upgrade, and none would have helped. The 2026-09-19 probe read the key as taking prefixes where the tool takes components (F-097) | nothing; the capability was there | **closed.** Generated and vendored files left the denominator and `coupling.bypass_prefixes` is still empty, so nothing lost `C-001` |
| W-15 | integration evidence surface: `hiqlite/tests/cluster/` (A19, F-017) | **`012-cluster-integration-evidence`**, 2026-09-21 | **delivered at M1**: `012` establishes the fifteen files `002` did not claim; no `spec-spine.toml` change was needed, because the directory was already declared by the pilot | M2 partly: `012` B-1 maps all fifteen phases to the guarantee each establishes, and section 4 states what the suite cannot reach, which is any phase independently, anything under TLS, and anything needing two processes | none outstanding | none | **closed at M1.** The stall is **diagnosed by execution**, not restated: F-051 records a connect-versus-publish race in `Client::remote` with a measured 175 ms losing window. Eight findings recorded (F-048 to F-055), F-017's first half closed and its second half given a dated disposition. **Closed by execution on 2026-09-22**: `032` repairs F-051 and the fifteen phases complete, which is also how F-100 was found |
...
| W-17 | public API and error taxonomy (A18) | partly, by the repair specs that added to it | **the lifecycle surface is delivered** by `027` B-7: `Error::Startup`, `Error::NodeFailed`, `Client::node_failure`, `Client::ensure_node_available`, and a read-only `hiqlite::lifecycle` module. The other error variants this release adds are `Error::CacheIncompatible` (`022`) and `Error::StorageInUse` (`024`) | behavioral assertions exist for each new variant in its own spec's block | **answered narrowly** (`027` B-7): the lifecycle surface is frozen for this release and nothing else is. No readiness enum, health-check trait, restart policy or supervision hook is frozen | none | partly closed. The error semantics of the four new variants are specified with consumer-facing assertions; a full taxonomy over the pre-existing API is not attempted |
...
| W-22 | startup-error and background-task lifecycle policy: F-009, F-014, F-025, and F-039 and F-040 added by `010` | **`027-node-lifecycle-and-startup-errors`**, 2026-09-21 | **delivered**: expected failures are returned as named errors, both listeners are bound before the node is reported started and shut down gracefully on both paths, a partial startup tears down what it started, and a WAL writer that ends for any reason takes the node out of service | three unit tests under unwind, **plus `hiqlite-abort-probe`**, a `--release` binary that inherits `panic = "abort"` and reports five expected failure paths returning errors (exit `0`); with one `.expect(..)` restored it aborted with exit `134`. No test can do this: Rust's harness requires unwinding | both answered in `027` B-1, B-5, D-1 and D-2: report and refuse, never exit, never restart automatically; and a listener that cannot bind is a startup error | W-01 and W-02, both delivered | **closed.** The policy is externally observable and is characterized under **both** profiles, which is the part that needed a binary rather than a test. Not closed, and stated as such: no node is started in any test, so the bind ordering, the TLS shutdown and the teardown are source changes; and six defects are recorded (`027` KD-1 to KD-6), the first being that the crate still panics in places this spec did not sweep |
