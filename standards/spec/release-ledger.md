...
| R-23 | F-110: an out-of-service node refuses its embedded client | implemented | `027` B-9; shipped in `0.15.0-patched.1`, but no test here drives it, so the state stays where the evidence is; the confirming evidence is the Rauthy integration's WAL-failure injection against the published build (`027` KD-10) |
