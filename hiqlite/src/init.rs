...
async fn ask_peer(
    ...
    let body = match time::timeout(rest, resp.bytes()).await {
        Ok(Ok(body)) => body,
        Ok(Err(err)) => {
            return PeerAnswer::Silent(format!("answered {status}, body failed: {err}"));
        }
    ...
    match deserialize::<Membership<NodeId, Node>>(&body) {
        ...
        Err(err) => PeerAnswer::Silent(format!(
            "answered 200 with a body that is not a membership: {err}"
        )),
    }
}
...
mod tests {
    ...
    async fn f118_at_n5_two_distinct_peers_must_answer() {
        ...
        let err = should_node_1_skip_init(&some_raft_type(), &one, SECRET, false, false, F118_WAIT)
            .await
            .expect_err("one of the two answers needed");
        let text = err.to_string();
        assert!(
            text.contains("needs 2 peer(s)") && text.contains("1 did"),
            "got: {text}"
        );
        assert!(
            !text.contains("node 2 ("),
            "node 2 answered and is not silent: {text}"
        );
    ...
    async fn f118_unreachable_peers_are_not_evidence_of_a_fresh_cluster() {
        ...
        assert!(
            ...
            "a startup error, got: {text}"
        );
        assert!(
            text.contains("node 2") && text.contains("node 3"),
            "names the silent peers: {text}"
        );
    }
    ...
    async fn f118_an_unauthenticated_answer_is_not_evidence() {
        ...
        assert!(
            ...
            "says why the peer did not count: {err}"
        );
    }
